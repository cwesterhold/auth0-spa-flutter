...
import 'package:auth0_spa/src/definitions/user.dart';
...
class Auth0ClientJs extends Auth0Client {
  ...
  void loginWithRedirect([RedirectLoginOptions options]) {
    this
        ._jsAuth0Client
        .invokeAsyncMethod<void>("loginWithRedirect", <dynamic>[options?.toJsObject()]);
  }
  ...
  Future<void> loginWithPopup([PopupLoginOptions options]) {
    return this
        ._jsAuth0Client
        .invokeAsyncMethod<void>("loginWithPopup", <dynamic>[options?.toJsObject()]);
  }

  @override
  Future<Map<String, String>> handleRedirectCallbackWithQuery(String searchQuery,
    ...
    final result = await this
        ._jsAuth0Client
        .invokeAsyncMethod<JsObjectBridge>("handleRedirectCallback", <dynamic>[searchQuery]);
  ...
  @override
  Future<dynamic> getUser([GetUserOptions options]) async {
    JsObjectBridge userJs =
        await this._jsAuth0Client.invokeAsyncMethod("getUser", <dynamic>[options?.toJsObject()]);

    try {
      return User(
        sub: userJs.get("sub"),
        name: userJs.get("name"),
        nickname: userJs.get("nickname"),
        picture: userJs.get("picture"),
        updated_at: userJs.get("updated_at"),
      );
    } catch (e) {
      return null;
    }
  ...
  Future<String> getTokenSilently([GetTokenSilentlyOptions options]) async {
    try {
      final result = await this
          ._jsAuth0Client
          .invokeAsyncMethod<String>("getTokenSilently", <dynamic>[options?.toJsObject()]);
      return result;
    } catch (e) {
  ...
  Future<IdToken> getIdTokenClaims([GetIdTokenClaimsOptions options]) async {
    JsObjectBridge userTokenJs = await this
        ._jsAuth0Client
        .invokeAsyncMethod("getIdTokenClaims", <dynamic>[options?.toJsObject()]);
    return IdToken(
  ...
  Future<String> getTokenWithPopup([GetTokenWithPopupOptions options]) {
    return this
        ._jsAuth0Client
        .invokeAsyncMethod("getTokenWithPopup", <dynamic>[options?.toJsObject()]);
  }
